...
typedef struct{
  ...
  uint16_t  CustomImuHdle;                  /**< IMU handle */
 ...
 */
#define COPY_IMU_DATA_UUID(uuid_struct)          COPY_UUID_128(uuid_struct,0x00,0x00,0x12,0x34,0xcc,0x7a,0x48,0x2a,0x98,0x4a,0x7f,0x2e,0xd5,0xb3,0xe5,0x8f)
#define COPY_IMU_UUID(uuid_struct)    COPY_UUID_128(uuid_struct,0x00,0x00,0x56,0x78,0x8e,0x22,0x45,0x41,0x9d,0x4c,0x21,0xed,0xae,0x82,0xed,0x19)
...
void SVCCTL_InitCustomSvc(void)
{
   ...
   * service_max_attribute_record = 1 for IMU_Data +
   *                                2 for IMU +
   *                                1 for IMU configuration descriptor +
   *                              = 4
   *
  ...
  {
    APP_DBG_MSG("  Fail   : aci_gatt_add_service command: IMU_Data, error code: 0x%x \n\r", ret);
  }
  else
  {
    APP_DBG_MSG("  Success: aci_gatt_add_service command: IMU_Data \n\r");
  }
  ...
  ret = aci_gatt_add_char(CustomContext.CustomImu_DataHdle,
                          ...
                          &(CustomContext.CustomImuHdle));
  if (ret != BLE_STATUS_SUCCESS)
  {
    APP_DBG_MSG("  Fail   : aci_gatt_add_char command   : IMU, error code: 0x%x \n\r", ret);
  }
  else
  {
    APP_DBG_MSG("  Success: aci_gatt_add_char command   : IMU \n\r");
  }
  ...
  switch (CharOpcode)
  {
    ...
    case CUSTOM_STM_IMU:
      ret = aci_gatt_update_char_value(CustomContext.CustomImu_DataHdle,
                                       CustomContext.CustomImuHdle,
                                       0, /* charValOffset */
                                       SizeImu, /* charValueLen */
                                       (uint8_t *)  pPayload);
      if (ret != BLE_STATUS_SUCCESS)
      {
        APP_DBG_MSG("  Fail   : aci_gatt_update_char_value IMU command, result : 0x%x \n\r", ret);
      }
      else
      {
