...
public class HandUpdate2 : MonoBehaviour
{
    IMU_SerialPort IMU_SP;
    ...
    public List<Quaternion> Correct_q = new List<Quaternion>();
    ...
    void Start()
    {
        IMU_SP = gameObject.AddComponent<IMU_SerialPort>();


        for (int index = 0; index < IMU_SP.Number_Of_IMU; index++)
        {
            MahonyFilters.Add(new Mahony());
            Correct_q.Add(new Quaternion(0, 0, 0, 1));
        }
    ...
    void Update()
    {
        // Update hand rotation first, then update each node (from pinky to thumb)
        if (IMU_SP.IMU_DataParse() == true)
        {
            /*   Update hand rotation   */

            // Cause hardware circuit (Hand) design need custom rotate IC orientation
            Acc = Quaternion.AngleAxis(-90, new Vector3(0, 0, 1)) * IMU_SP.Acc[0];
            Gyro = Quaternion.AngleAxis(-90, new Vector3(0, 0, 1)) * IMU_SP.Gyro[0];

            // Base on Mahony filter algorithm update filter system q
            Quaternion Hand_q = World_To_Unity_Rotation(MahonyFilters[0].update_q(Acc, Gyro));
            ...
            Hand.rotation = Correct_q[0] * Hand_q * Initial_q[0];

            //Debug.Log("Hand Acc x: " + Acc.x + ", y: " + Acc.y + ", z: " + Acc.z);
            //Debug.Log("Hand q.w: " + Hand_q.w + ", q.x: " + Hand_q.x + ", q.y: " + Hand_q.y + ", q.z: " + Hand_q.z);
            ...
            {
                // Cause hardware circuit (Hand) design need custom rotate IC orientation
                Acc = Quaternion.AngleAxis(90, new Vector3(0, 0, 1)) * IMU_SP.Acc[index];
                Gyro = Quaternion.AngleAxis(90, new Vector3(0, 0, 1)) * IMU_SP.Gyro[index];

                // Base on Mahony filter algorithm update filter system q
                Quaternion node_q = World_To_Unity_Rotation(MahonyFilters[index].update_q(Acc, Gyro));
                ...
                if (index != 1)
                {
                    ...
                    // Correct quaterion first (Inverse rotation last time record), then rotate current base mahony quaternion. Finally recorver to normal initial rotation.
                    Nodes[index].rotation = Correct_q[index] * diff_hand_node_q * Hand_q * Initial_q[index];
                    Nodes[index].transform.GetChild(0).rotation = Correct_q[index] * diff_hand_node_q * diff_hand_node_q * Hand_q * Initial_q[index];
                    Nodes[index].transform.GetChild(0).GetChild(0).rotation = Correct_q[index] * diff_hand_node_q * diff_hand_node_q * diff_hand_node_q * Hand_q * Initial_q[index];
                }
                else
                {
                    ...
                    // Correct quaterion first (Inverse rotation last time record), then rotate current base mahony quaternion. Finally recorver to normal initial rotation.
                    Nodes[index].transform.rotation = Correct_q[index] * diff_hand_node_pitch_q * diff_hand_node_yaw_q * Hand_q * Initial_q[index];
                }
            }
        }
    }

    public void HandCorrect()
    {
        for (int index = 0; index < IMU_SP.Number_Of_IMU; index++)
        {
            Correct_q[index] = Quaternion.Inverse(World_To_Unity_Rotation(MahonyFilters[index].q));
        }
    }

    Quaternion World_To_Unity_Rotation(Quaternion q)
    {
